...
const CheckInsTab: React.FC<CheckInsTabProps> = ({ showAlert, apiBaseUrl, navigate }) => {
  ...
  const refreshTicketInfo = async (code: string): Promise<void> => {
    try {
      ...
      const response = await axios.get(
        `${apiBaseUrl}/api/Ticket/code/${code}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="mb-6">
            <div className="flex space-x-2">
              <div className="relative flex-1">
                <input
                  type="text"
                  placeholder="Nhập mã vé (ví dụ: 7C4119B9)"
                  value={ticketCode}
                  ...
                  return (
                    <div 
                      ...
                      className={`border rounded-lg overflow-hidden hover:shadow-md transition-shadow ${
                        ticketInfo?.ticket_code === ticket.ticket_code ? 'ring-2 ring-indigo-500' : ''
                      }`}
                    >
      ...
      {/* Initial Empty State */}
      {!ticketInfo && !isLoading && !ticketCode && !isLoaded && (
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          <div className="text-center py-12">
