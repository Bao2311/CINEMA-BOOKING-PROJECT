...
//     if (props.seatStatus === "Sold" || props.seatStatus === "Reserved")
//       return "#9ca3af";
...
//   font-size: 0.8rem;
//   cursor: ${(props) =>
//     props.seatStatus === "NotAvailable" ||
//     props.seatStatus === "Sold" ||
//     props.seatStatus === "Reserved" ||
//     props.seatStatus === "Booked"
//       ? "not-allowed"
//       : "pointer"};
//   position: relative;
//   transition: all 0.3s ease;
...
//   const [memberLookupType, setMemberLookupType] = useState<"phone" | "email">(
//     "phone"
//   );
...
//   const [newUserModalVisible, setNewUserModalVisible] =
//     useState<boolean>(false);
//   const [paymentQrVisible, setPaymentQrVisible] = useState<boolean>(false);
//   const [paymentData, setPaymentData] = useState<PayosPaymentResponse | null>(
//     null
//   );
...
//     PendingBookingResponse["pendingBooking"] | null
//   >(null);
//   const [cancelBookingModalVisible, setCancelBookingModalVisible] =
//     useState<boolean>(false);
//   const [selectedShowtimeToBook, setSelectedShowtimeToBook] =
...
//         "https://localhost:7168/api/Movie/now-showing",
//         {
//           headers: {
...
//           (promo: Promotion) =>
//             promo.status.toLowerCase() === "active" && promo.is_Active
//         );
...
//       const token = getAuthToken();
//       const response = await axios.get(
//         `https://localhost:7168/api/Showtimes/movie/${movieId}/dates`,
//         {
//           headers: {
//             Authorization: token ? `Bearer ${token}` : undefined,
//           },
...
//           fetchShowtimes(
//             movieId,
//             moment(response.data.$values[0]).format("YYYY-MM-DD")
//           );
//         }
//       } else if (Array.isArray(response.data)) {
...
//           fetchShowtimes(
//             movieId,
//             moment(response.data[0]).format("YYYY-MM-DD")
//           );
//         }
//       } else {
//         setAvailableDates([]);
...
//         seat_Quantity:
//           showtime.room?.seat_Quantity || showtime.seat_Quantity || 0,
//       }));
...
//       setLoading(true);
//       const token = getAuthToken();
//       const response = await axios.get<ShowtimeSeatsResponse>(
//         `https://localhost:7168/api/Seat/showtime/${showtimeId}`,
//         {
//           headers: {
//             Authorization: token ? `Bearer ${token}` : undefined,
//           },
...
//               (s) => s.row_Name === row && s.seat_Number === col
//             );
...
//                 ...existingSeat,
//                 seat_ID:
//                   existingSeat.seat_ID === 0
//                     ? existingSeat.layout_ID
//                     : existingSeat.seat_ID,
//               };
//             }
...
//         console.log(
//           `Tổng số hàng (A-${String.fromCharCode(
//             "A".charCodeAt(0) + maxRow
//           )}): ${allRows.length}`
//         );
...
//         message.info(
//           `Sơ đồ ghế: ${allRows.length} hàng (A-${String.fromCharCode(
//             "A".charCodeAt(0) + maxRow
//           )}) × ${allColumns.length} cột (1-${maxCol})\n` +
//             `Tổng số ghế: ${fullSeats.length}\n` +
//             `Ghế có sẵn: ${
//               fullSeats.filter((s) => s.seat_Status === "Available").length
//             }\n` +
//             `Ghế đã đặt: ${
//               fullSeats.filter(
//                 (s) =>
...
//       const endpoint =
//         type === "phone"
//           ? `https://localhost:7168/api/Member/lookup/phone/${encodeURIComponent(
//               value
//             )}`
//           : `https://localhost:7168/api/Member/lookup/email/${encodeURIComponent(
//               value
//             )}`;
...
//         message.success(
//           `Đã tìm thấy thành viên ${response.data.full_Name} - ${
//             response.data.currentPoints?.toLocaleString() || 0
//           } điểm`
//         );

//         // New API call
//         const bookingResponse = await axios.post(
//           "https://localhost:7168/api/Member/link-member",
//           {
...
//             message.info(
//               `Số điểm hiện tại: ${
//                 bookingResponse.data.currentPoints?.toLocaleString() || 0
//               } điểm`
//             );
//           }
//         }
...
//       if (linked) {
//         message.success(
//           "Đã áp dụng thông tin thành viên và liên kết với đơn đặt vé!"
//         );
//       } else {
...
//       const token = getAuthToken();
//       const response = await axios.get(
//         `https://localhost:7168/api/Promotion/member-discount/${membershipLevel}`,
//         {
//           headers: {
//             Authorization: token ? `Bearer ${token}` : undefined,
//           },
...
//   const linkMemberToBooking = async (
//     bookingId: number,
...
//           message.info(
//             `Số điểm hiện tại của thành viên: ${
//               response.data.currentPoints?.toLocaleString() || 0
//             } điểm`
//           );
//         }
...
//       message.warning(
//         "Vui lòng nhập số điểm cần sử dụng và đảm bảo đã tạo đơn đặt vé"
//       );
//       return;
//     }
...
//       message.warning(
//         `Bạn chỉ có thể sử dụng tối đa ${maxAllowedPoints.toLocaleString()} điểm (50% tổng hóa đơn)`
//       );
//       return;
//     }
...
//           setMember((prev) =>
//             prev
//               ? { ...prev, currentPoints: response.data.currentPoints }
//               : prev
//           );
//         }
...
//         message.success(
//           `Đã sử dụng ${pointsToUse.toLocaleString()} điểm để giảm giá ${discountAmount.toLocaleString()} VND`
//         );
...
//       const token = getAuthToken();
//       const response = await axios.post(
//         "https://localhost:7168/api/User/staff-register",
//         {
...
//         const response = await axios.put(
//           `https://localhost:7168/api/Booking/${bookingId}/payment`,
//           {},
//           {
//             headers: {
//               Authorization: token ? `Bearer ${token}` : undefined,
...
//         bookingId: bookingId,
//         movieName: selectedMovie?.movie_Name,
//         showtime: `${formatDate(selectedShowtime?.show_Date)} ${formatTime(
...
//           (s) => s.row_Name === rowName && s.seat_Number === colNum
//         );
//         return (
//           seat || {
//             seat_ID: -1,
...
//             />
//             <Tag
//               color={movie.rating === "P13" ? "orange" : "green"}
...
//             </Space>
//             <div className="mt-2">
//               <Text strong>Suất chiếu:</Text> {formatDate(showtime?.show_Date)}{" "}
...
//             </div>
//             <div>
//               <Text strong>Giá:</Text> {showtime?.base_Price.toLocaleString()}{" "}
//               VND
//             </div>
//             <div className="mt-2">
//               <Tag color="green">
...
//                   seats.filter(
//                     (s) =>
//                       s.seat_Status !== "Booked" && s.seat_Status !== "Reserved"
//                   ).length
//                 }{" "}
//                 ghế trống
//               </Tag>
...
//             <Tag color="green">
//               {member.currentPoints?.toLocaleString() || 0} điểm
//             </Tag>
//           </Descriptions.Item>
...
//             } - Giảm: ${appliedPromotion.discount_Amount.toLocaleString()} VND`}
//             type="success"
//             showIcon
//             className="mb-4"
//           />
...
//                   </p>
//                   <p>
//                     <strong>Diễn viên:</strong> {selectedMovie.cast}
//                   </p>
//                 </Col>
//                 <Col span={12}>
//                   <p>
...
//                     <Tag
//                       color={
//                         selectedMovie.rating === "P13" ? "orange" : "green"
//                       }
//                     >
...
//                 <ColumnFooter>
//                   {getUniqueColumnNumbers().map((colNum) => (
//                     <ColumnLabel key={`col-footer-${colNum}`}>
//                       {colNum}
//                     </ColumnLabel>
//                   ))}
//                 </ColumnFooter>
...
//                     {formatDate(selectedShowtime.show_Date)}{" "}
//                     {formatTime(selectedShowtime.start_Time)} -{" "}
//                     {formatTime(selectedShowtime.end_Time)}
//                   </Descriptions.Item>
...
//                 type="primary"
//                 onClick={() =>
//                   lookupMember(memberLookupValue, memberLookupType)
//                 }
...
//                           max={Math.min(
//                             member.currentPoints,
//                             Math.floor(bookingSummary.subtotal * 0.5)
//                           )}
//                           style={{ width: "60%" }}
//                         />
//                         <Button
...
//                       <Text>
//                         Giảm giá thành viên ({memberDiscountAmount}%):
//                       </Text>
//                       <Text>
//                         -{bookingSummary.memberDiscount.toLocaleString()} VND
//                       </Text>
//                     </div>
//                   )}
...
//         footer={null}
//       >
//         <Form form={newUserForm} layout="vertical" onFinish={registerNewMember}>
//           <Form.Item
//             name="name"
//             label="Họ và tên"
...
//           <Form.Item name="sex" label="Giới tính" initialValue="Other">
//             <Select>
//               <Option value="Male">Nam</Option>
//               <Option value="Female">Nữ</Option>
//               <Option value="Other">Khác</Option>
...
//             <div className="mb-4">
//               <QRCode
//                 value={paymentData.qrCodeUrl || paymentData.paymentUrl}
//                 size={250}
//                 bordered
//               />
//             </div>
...
//               <br />
//               Sau khi thanh toán thành công, vui lòng nhấn "Đã thanh toán thành
//               công".
//             </Paragraph>
//             <div className="mt-4">
...
//           <Button
//             key="cancel"
//             onClick={() => setCancelBookingModalVisible(false)}
//           >
//             Không hủy
//           </Button>,
//           <Button
...
//                     <li>
//                       Suất chiếu:{" "}
//                       {moment(pendingBooking.show_Date).format("DD/MM/YYYY")}{" "}
//                       {pendingBooking.start_Time}
//                     </li>
//                     <li>Phòng: {pendingBooking.roomName}</li>
//                     <li>Ghế: {pendingBooking.seats}</li>
//                     <li>
//                       Tổng tiền: {pendingBooking.total_Amount.toLocaleString()}{" "}
//                       VND
//                     </li>
//                     <li>
//                       Thời gian còn lại: {pendingBooking.remainingMinutes} phút
//                     </li>
//                   </ul>
//                   <p className="mt-2">
//                     Bạn có muốn hủy đơn đặt vé này để tiếp tục đặt vé mới không?
//                   </p>
//                 </div>
//               }
//             />
