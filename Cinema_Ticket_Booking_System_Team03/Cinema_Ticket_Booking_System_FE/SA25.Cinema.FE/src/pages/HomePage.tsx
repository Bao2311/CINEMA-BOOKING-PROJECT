    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get('https://localhost:7168/api/Movie', {
          headers: {
            Authorization: `Bearer ${token}`, // Token được truyền vào header
          },
        });

        if (response.data && Array.isArray(response.data.$values)) {
          // Sắp xếp phim nổi bật theo xếp hạng
          ...
          const upcoming = response.data.$values.filter((movie: Movie) => {
            return new Date(movie.release_Date) > today; // Kiểm tra phim có release_Date sau ngày hôm nay không
          });
          setUpcomingMovies(upcoming.slice(0, 4)); // Hiển thị tối đa 4 phim sắp chiếu
        } else {
